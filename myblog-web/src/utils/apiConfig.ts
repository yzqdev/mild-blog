...
export const getTimeline = (x:{pageNum:number,pageSize:number}) => {
    return axios.post("/home/timeline", {pageNum: x.pageNum, pageSize: x.pageSize});
...
export const getBlogList = (data:{pageNum:number,pageSize:number}) => {
    return axios.get(`/admin/blog/list?pageNum=${(data.pageNum)}&pageSize=${(data.pageSize)}`);
};
export const getCommentList = (data:{page:number,limit:number}) => {
    return axios.get(`/admin/comment/paging?page=${(data.page)}&limit=${(data.limit)}`);
};
...
export const getCatePaging = (data:{page:number,limit:number}) => {
    return axios.get(`/admin/category/paging?page=${(data.page)}&limit=${(data.limit)}`);
};
